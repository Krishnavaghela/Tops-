...
create table if not exists sales (sale_id int primary key,product_name varchar(50),category varchar(50),sale_amount decimal(10,2),sale_date date);
...
select max(sale_amount),category 
from sales 
group by category;
...
select count(*) as sale,product_name 
from sales
...
select category,product_name,avg(sale_amount) 
from sales
...
select product_name,count(*), month(sale_date) 
from sales
...
select count(*),category,month(sale_date),avg(sale_amount) 
from sales
...
from sales
group by sale_amount,product_name
...
select category,month(sale_date),sum(sale_amount) 
from sales
group by month(sale_date),category
...
from sales
group by product_name
having max(sale_amount) >avg(sale_amount);
...
from sales
group by product_name
having min(sale_amount)<avg(sale_amount);
...
select category,count(*),sale_amount 
from sales
group by category,sale_amount
having count(*)>=1 and sale_amount>200;
...
select distinct product_name,month(sale_date),count(*) 
from sales
...
select month(sale_date) as month, 
       (max((sale_amount)) - min((sale_amount))) / min((sale_amount)) as sales_increase_percentage
from sales
group by month(sale_date)
...
select product_name,month(sale_date),count(*),sale_amount from sales
group by product_name,month(sale_date),sale_amount
having count(*)>2 and sale_amount<30;
...
select A.product_name as product_name1, B.product_name as product_name2, A.sale_amount
from sales A, sales B
where A.sale_id <> B.sale_id
and A.sale_amount = B.sale_amount
and A.category != B.category
order by A.sale_amount;
